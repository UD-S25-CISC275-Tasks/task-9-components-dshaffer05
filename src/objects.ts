...
export function isCorrect(question: Question, answer: string): boolean {
    let goodAns: string = answer.trim();
    let answerLow = goodAns.toLowerCase();
    let expectedLow = question.expected.toLowerCase();
    if (expectedLow === answerLow) {
        return true;
    } else {
        return false;
    }
...
export function isValid(question: Question, answer: string): boolean {
    ...
    let hasAnswer: boolean = question.options.includes(answer);
    return hasAnswer;
}
...
export function toShortForm(question: Question): string {
    ...
    let trimmed = name.slice(0, 10);
    let short = id + ": " + trimmed;
    return short;
}
